...
def build_schema(columns: List[str]) -> Dict[str, pl.DataType]:
    return {col: TYPE_MAPPING[RAW_TYPE_DICT[col]] for col in columns if col in RAW_TYPE_DICT}
...
def process_file(input_path: Path, output_path: Path, schema: Dict[str, pl.DataType]):
    try:
        ...
        df = df.with_columns(
                [
                    pl.col('ANARCI_numbering_light').str.replace_all("'", '"').str.replace_all(' ', ''),
                    pl.col('ANARCI_numbering_heavy').str.replace_all("'", '"').str.replace_all(' ', '')
                ]
            )
